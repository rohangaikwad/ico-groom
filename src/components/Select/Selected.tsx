...
const SelectedIcons: Component = () => {
    ...
    const removeIcon = (catName: keyof ISelectedType, icon: IIconType) => {
        ...
        let selIndex = _selected[catName].findIndex(s => s.defaultCode === icon.defaultCode);
    ...
    return <section id="selected">
        ...
        {Object.values(selected()).flat().length > 0 && <button id="reset" onClick={() => removeAll()}>
        ...
        {Object.values(selected()).map((fontIcons: IIconType[], i) => {
            ...
            return fontIcons.length > 0 && <div class={`${catName} category`}>
                <h5>{catName}</h5>
                <ul>
                    {fontIcons.map((icon) => <li onClick={() => removeIcon(catName, icon)}>
