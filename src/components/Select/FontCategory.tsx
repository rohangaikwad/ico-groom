...
const FontCategory: Component<{
  ...
  const [itemsPerRow, setItemsPerRow] = createSignal(Math.floor(rowWidth() / ((iconSize() * 3) + 10)));
  ...
  window.addEventListener("resize", () => {
    let width = document.querySelector(".icon-list-scroll > .icon-list")?.clientWidth || 300;
    setRowWidth(width);
  });
        ...
        <div ref={scrollParentRef} class="icon-list-scroll">          
          <For each={rowVirtualizer.getVirtualItems()}>
            {(virtualItem: IVirtualItem) => {
              ...
              >
                {fIcons().slice(virtualItem.index * itemsPerRow(), (virtualItem.index + 1) * itemsPerRow()).map((icon) => <li data-code={icon?.defaultCode} onClick={() => toggleSelection(icon)} class={IsSelected(icon) ? "active" : ""} >
                  <svg width={32} height={32} viewBox={`0 0 ${icon.width || 1024} 1024`}>
                    {icon.paths.map((path) => <path d={path} /> )}
                  </svg>
                  <span>{icon?.tags[0] || ""} - {selected()[categoryName].findIndex((s) => s.defaultCode === icon.defaultCode)}</span>
                </li>)}
              </ul>
            }}
          </For>
